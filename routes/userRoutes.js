...
userRoute.get("/me/following", passport.authenticate("jwt", { session: false }), async (req, res) => {

  try {
    const followingRecord = await Follow.find({ follower: req.user }).populate("following", "username bio profilePic");
    res.json({
...
userRoute.get("/me/followers", passport.authenticate("jwt", { session: false }), async (req, res) => {


  try { 
    const followersRecord = await Follow.find({ following: req.user }).populate("follower", "username bio profilePic");
